  ...
  try {
    ...
    const fetchClients = async () => {
      ...
      try {
        ...
        // FIX: Handle both direct array and { success, clients } structure
                    ...
                    <table className="w-full text-left border-collapse">
                       ...
                       <tbody className="divide-y divide-slate-100 bg-white">
                          {(clients || []).map((client: any) => (
                             <tr key={client?.id || Math.random()} className="hover:bg-slate-50 transition-colors group">
                                <td className="px-8 py-5 font-semibold text-sm text-slate-900">{client?.name || 'Unknown'}</td>
                                <td className="px-8 py-5 text-sm text-slate-500">{client?.companyName || client?.company_name || '-'}</td>
                                <td className="px-8 py-5 text-sm text-slate-500">{client?.email || 'No email'}</td>
                                <td className="px-8 py-5">
                                   <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold border uppercase tracking-tighter ${
                                      (client?.status || 'ACTIVE').toLowerCase() === 'active' ? 'bg-green-50 text-green-700 border-green-100' : 'bg-slate-50 text-slate-700 border-slate-200'
                                   }`}>
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-8">
        <div className="card max-w-md w-full text-center p-10">
          ...
          <p className="text-slate-500 text-sm mb-6">The page encountered a client-side error.</p>
          <button 
            onClick={() => window.location.reload()}
            className="btn-primary w-full"
          >

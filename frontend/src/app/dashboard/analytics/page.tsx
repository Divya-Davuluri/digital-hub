...
import { 
  ...
  Target, BarChart3, Download, Activity, Globe, Mail
} from 'lucide-react';
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {campaigns.map((camp) => (
                      <tr key={camp.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-8 py-4 text-sm font-bold text-slate-900">${camp.budget.toLocaleString()}</td>
                        <td className="px-8 py-4 text-right">
                          <div className="flex flex-col items-end gap-1.5">
                            <span className="text-sm font-bold text-slate-900">${camp.spent.toLocaleString()}</span>

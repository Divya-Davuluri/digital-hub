...
export default function Reset2FAPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiFetch('/auth/forgot-2fa-request', {
        method: 'POST',
        body: JSON.stringify({ email }),
      });

      setMessage({ type: 'success', text: '2FA reset link sent! Please check your email (or see token below in dev mode).' });
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-background px-6 relative overflow-hidden">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full -z-10">
        <div className="absolute top-1/4 left-1/4 w-[60%] h-[60%] bg-secondary/10 blur-[120px] rounded-full" />
      </div>
      ...
      <div className="w-full max-w-[440px] card p-10">
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl font-extrabold text-white tracking-tight">Lost 2FA Access?</h2>
          <p className="text-text-muted mt-2 text-sm">Enter your email and we&apos;ll send you a link to disable 2FA.</p>
        </div>
        ...
        {devToken && (
          <div className="mb-6 p-4 rounded-xl bg-white/5 border border-white/10">
             ...
             <Link href={`/reset-2fa/confirm?token=${devToken}`} className="text-xs text-text break-all hover:underline font-medium">
               http://localhost:3000/reset-2fa/confirm?token={devToken}
             </Link>
          </div>
        )}
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          >
            {loading ? "Sending..." : "Send 2FA Reset Link"}
          </button>
        </form>
      </div>
    </main>

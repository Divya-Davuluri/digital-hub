 ...
 */
export const downloadClientReport = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.user.tenantId;
    
    // In a real app, verify reportId belongs to client. Here we dynamically generate a generic report
    // as per step 2 instructions since physical files aren't stored yet.
    ...
    doc.fontSize(24).font('Helvetica-Bold').fillColor(branding?.primaryColor || '#4f46e5').text('Agency Performance Report', { align: 'center' });
    doc.moveDown(0.2);
    doc.fontSize(10).font('Helvetica').fillColor('#64748b').text(`Generated on ${new Date().toLocaleDateString()}`, { align: 'center' });
    doc.moveDown(2);
    ...
    doc.fontSize(14).font('Helvetica-Bold').fillColor('#0f172a').text('Campaigns List');
    doc.moveDown(1);
 ...
 */
export const getReportRequests = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.user.tenantId;
    ...
    .where(eq(reportRequests.tenantId, tenantId))

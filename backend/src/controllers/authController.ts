...
import { z } from 'zod';
import { db } from '../db';
import { users, backupCodes, sessions, resetTokens } from '../db/schema';
...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    res.status(201).json({ token, refreshToken, user: { id: userId, email, name } });
  } catch (error: any) {
...
export const setup2FA = async (req: any, res: Response) => {
  try {
    ...
    // Store TEMP secret in database (encrypted)
    const encryptedTempSecret = encrypt(secret.base32);
    await db.update(users).set({ 
      twoFactorTempSecret: encryptedTempSecret 
    }).where(eq(users.id, user.id));

    console.log(`[2FA SETUP] Generated temp secret for user ${user.id}`);
...
export const verify2FA = async (req: any, res: Response) => {
  try {
    ...
    // Fetch user from DB to get the latest temp secret
    ...
    console.log(`[2FA VERIFY] User: ${user.id}, Entered Code: ${token}, Temp Secret: ${tempSecret}`);
    ...
    await db.update(users).set({ 
      twoFactorEnabled: 1,
      twoFactorSecret: dbUser.twoFactorTempSecret, // Already encrypted
...
export const validate2FA = async (req: Request, res: Response) => {
  try {
    ...
    console.log(`[2FA LOGIN] User: ${userId}, Code: ${token}, Result: ${isTotpValid}`);
    ...
    if (!isValid) {
      ...
      for (const bc of userBackupCodes) {
        ...
        if (match) {
          ...
          console.log(`[2FA LOGIN] Backup code used successfully for user ${userId}`);
          break;
        }
      }
    }
    ...
    if (error instanceof z.ZodError) {
      return res.status(400).json({ message: error.issues[0]?.message || 'Verification failed' });
    }
...
export const forgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    // In a real app, send email. Here, return token for dev.
...
export const resetPassword = async (req: Request, res: Response) => {
  try {
    ...
    const resetToken = await db.query.resetTokens.findFirst({
      where: and(eq(resetTokens.token, token), eq(resetTokens.type, 'password')),
    });
...
export const updateProfile = async (req: any, res: Response) => {
  try {
    ...
    if (error instanceof z.ZodError) {
      return res.status(400).json({ message: error.issues[0]?.message || 'Validation failed' });
    }
...
export const reset2FAConfirm = async (req: Request, res: Response) => {
  try {
    const { token } = req.body;
    const resetToken = await db.query.resetTokens.findFirst({
      where: and(eq(resetTokens.token, token), eq(resetTokens.type, '2fa')),
    });
